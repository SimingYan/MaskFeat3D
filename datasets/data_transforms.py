...
class PointcloudNormalRotate(object):
    def __call__(self, data):
        ...
        for i in range(bsize):
            ...
            normal[i, :, :] = torch.matmul(normal[i], R)
...
class PointcloudSingleScaleAndTranslate(object):
    def __init__(self, scale_low=2. / 3., scale_high=3. / 2., translate_range=0.2):
        self.scale_low = scale_low
        self.scale_high = scale_high
        self.translate_range = translate_range
    ...
    def __call__(self, data):
        ...
        bsize = pc.size()[0]
        for i in range(bsize):
            xyz1 = np.random.uniform(low=self.scale_low, high=self.scale_high, size=[1])
            xyz2 = np.random.uniform(low=-self.translate_range, high=self.translate_range, size=[3])
...
class PointcloudNormalJitter(object):
    def __init__(self, std=0.01, clip=0.05):
        self.std, self.clip = std, clip
...
class PointcloudSingleScale(object):
    def __init__(self, scale_low=2. / 3., scale_high=3. / 2.):
        self.scale_low = scale_low
        self.scale_high = scale_high
...
class PointcloudRandomInputDropout(object):
    ...
    def __call__(self, pc, normal=None):
        bsize = pc.size()[0]
        for i in range(bsize):
