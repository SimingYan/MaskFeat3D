...
from lib.pad_voxel_for_ti import pad_voxel_at_strides
...
class Voxelizer:
  def __init__(self,
    ...
    self.voxel_pad_strides = voxel_pad_strides
  ...
  def voxelize(self, coords, feats, labels, center=None):
    ...
    pc_coords = coords_aug - min_coords
    coords_aug = np.floor(pc_coords)
    ...
    if not type(coords_aug).__module__ == np.__name__:
        coords_aug = coords_aug.numpy()
 
    if len(self.voxel_pad_strides)>0:
      coords_aug, feats, labels = pad_voxel_at_strides(coords_aug, feats, labels, pc_coords, voxel_pad_strides=self.voxel_pad_strides, labels_pad_value=self.ignore_label)

    return coords_aug, feats, labels, rigid_transformation.flatten()

class Voxelizer_Point(Voxelizer):
  def voxelize(self, coords, feats, labels, center=None):
    assert coords.shape[1] == 3 and coords.shape[0] == feats.shape[0] and coords.shape[0]
    if self.clip_bound is not None:
      trans_aug_ratio = np.zeros(3)
    ...
    return coords_aug, feats, labels, rigid_transformation.flatten(), pc_coords, pc_labels
