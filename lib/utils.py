...
from lib.logger import print_log

def get_inner_weights(weights, prefix='module'):
  prefix = prefix + '.'
  if any([k.startswith(prefix) for k in weights.keys()]):
    logging.info("Loading weights with {} prefix...".format(prefix))
    inner_weights = {k.partition(prefix)[2]:weights[k] for k in weights.keys() if k.startswith(prefix)}
    left_weights = {k:weights[k] for k in weights.keys() if not k.startswith(prefix)}
  return inner_weights, left_weights
...
def load_state_with_same_shape(model, weights):
  ...
  if any([k.startswith('encoder.') for k in weights.keys()]):
    droped_weights = [k for k in weights.keys() if not k.startswith('encoder.')]
...
def checkpoint(fpath, model, optimizer, epoch, iteration, config, best_val=None, best_val_iter=None, postfix=None, logger=None):
  ...
  checkpoint_file = os.path.join(fpath, filename)
  print_log('Save checkpoint to %s' % checkpoint_file, logger=logger)
...
def fix_permute_pointcloud(input_coords, pointcloud, transformation, label_map,
                       voxel_output, voxel_pred):
  """Get permutation from pointcloud to input voxel coords."""
  def _hash_coords(coords, coords_min, coords_dim):
    return np.ravel_multi_index((coords - coords_min).T, coords_dim)
  # Validate input.
  input_batch_size = input_coords[:, 0].max().item()
  pointcloud_batch_size = pointcloud[:, 0].max().int().item()
  transformation_batch_size = transformation.shape[0] - 1
  assert input_batch_size == pointcloud_batch_size == transformation_batch_size
  pointcloud_permutation, pointcloud_target = [], []
  # Process each batch.
  for i in range(input_batch_size + 1):
    # Filter batch from the data.
    input_coords_mask_b = input_coords[:, 0] == i
    input_coords_b = (input_coords[input_coords_mask_b])[:, 1:].numpy()
    pointcloud_b = pointcloud[pointcloud[:, 0] == i, 1:].numpy()
    transformation_b = transformation[i].reshape(4, 4).numpy()
    # Transform original pointcloud to voxel space.
    original_coords1 = np.hstack((pointcloud_b[:, :3], np.ones((pointcloud_b.shape[0], 1))))
    original_vcoords = np.floor(original_coords1 @ transformation_b.T)[:, :3].astype(int)
    # Hash input and voxel coordinates to flat coordinate.
