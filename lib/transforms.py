...
class fix_cflt_collate_fn_factory:
  """Generates collate function for coords, feats, labels, point_clouds, transformations.
  ...
  def __call__(self, list_data):
    ...
    for transformation in transformations:
      if batch_id >= num_truncated_batch:
        break
      transformations_batch.append(torch.from_numpy(transformation[None:,]))
      batch_id += 1
  ...
  """Generates collate function for coords, feats, labels, point_clouds, transformations.
    ...
    Args:
      limit_numpoints: If 0 or False, does not alter batch size. If positive integer, limits batch
                       size so that the number of input coordinates is below limit_numpoints.
  """
  def __init__(self, num_of_class=16):
    self.num_of_class = num_of_class

  def __call__(self, list_data):
    coords, feats, labels, class_label = list(zip(*list_data))
    cfl_collate_fn = cfl_collate_fn_factory(None)
    coords_batch, feats_batch, labels_batch = cfl_collate_fn(list(zip(coords, feats, labels)))
    class_label = np.array(class_label, dtype=np.int32)
    class_label = np.eye(self.num_of_class)[class_label]
    class_label_batch = torch.from_numpy(class_label).float()
    return coords_batch, feats_batch, labels_batch, class_label_batch


class cflcpl_collate_fn_factory:
  """Generates collate function for coords, feats, labels, point_clouds, transformations.
  ...
  def __call__(self, list_data):
    coords, feats, labels, class_label, pc_coords, pc_labels = list(zip(*list_data))
    cflc_collate_fn = cflc_collate_fn_factory(self.num_of_class)
    coords_batch, feats_batch, labels_batch, class_label_batch = cflc_collate_fn(list(zip(coords, feats, labels, class_label)))
    batch_size = coords_batch[:, 0].max().item() + 1
    pc_batch = []
    pc_labels_batch = []
    for batch_id in range(batch_size):
      pc = pc_coords[batch_id]
      pc_batch.append(torch.cat([torch.ones(pc.shape[0], 1).float() * batch_id, torch.from_numpy(pc)], dim=1))
      pc_labels_batch.append(torch.from_numpy(pc_labels[batch_id]))
    pc_batch = torch.cat(pc_batch).float()
    pc_labels_batch = torch.cat(pc_labels_batch)
    return coords_batch, feats_batch, labels_batch, class_label_batch, pc_batch, pc_labels_batch

class cfltp_collate_fn_factory:
  """Generates collate function for coords, feats, labels, point_clouds, transformations.
  ...
  def __call__(self, list_data):
    coords, feats, labels, transformations, pc_coords, pc_labels = list(zip(*list_data))
    cfl_collate_fn = cfl_collate_fn_factory(limit_numpoints=self.limit_numpoints)
    coords_batch, feats_batch, labels_batch = cfl_collate_fn(list(zip(coords, feats, labels)))
    batch_size = coords_batch[:, 0].max().item() + 1
    ...
    for batch_id in range(batch_size):
      pc = pc_coords[batch_id]
      transformation = transformations[batch_id]
      pc_batch.append(torch.cat([torch.ones(pc.shape[0], 1).float() * batch_id, torch.from_numpy(pc)], dim=1))
      pc_labels_batch.append(torch.from_numpy(pc_labels[batch_id]))
      transformations_batch.append(torch.from_numpy(transformation[None,:]))
    ...
    pc_labels_batch = torch.cat(pc_labels_batch)
    transformations_batch = torch.cat(transformations_batch, 0).float()
    return coords_batch, feats_batch, labels_batch, transformations_batch, pc_batch, pc_labels_batch

class cfltpn_collate_fn_factory:
  """Generates collate function for coords, feats, labels, data_name.
    Args:
      limit_numpoints: If 0 or False, does not alter batch size. If positive integer, limits batch
                       size so that the number of input coordinates is below limit_numpoints.
  
    Note: It would be better to use for batch size = 1 case
  """
  def __init__(self, limit_numpoints):
    self.limit_numpoints = limit_numpoints

  def __call__(self, list_data):
    coords, feats, labels, transformations, pc_coords, pc_labels, data_name = list(zip(*list_data))
    coords_batch, feats_batch, labels_batch, transformations_batch, pc_batch, pc_labels_batch, data_name_batch = [], [], [], [], [], [], []
    ...
    for batch_id, _ in enumerate(coords):
      ...
      data_name_batch.append(data_name[batch_id]) 
    ...
    return coords_batch, feats_batch, labels_batch, transformations_batch, pc_batch, pc_labels_batch, data_name_batch

class cfltpni_collate_fn_factory:
  """Generates collate function for coords, feats, labels, data_name.
    Args:
      limit_numpoints: If 0 or False, does not alter batch size. If positive integer, limits batch
                       size so that the number of input coordinates is below limit_numpoints.
  ...
  def __call__(self, list_data):
    coords, feats, labels, transformations, pc_coords, pc_labels, data_name, index = list(zip(*list_data))
    coords_batch, feats_batch, labels_batch, transformations_batch, pc_batch, pc_labels_batch, data_name_batch, index_batch = [], [], [], [], [], [], [], []
    ...
    for batch_id, _ in enumerate(coords):
      ...
      data_name_batch.append(data_name[batch_id]) 
      index_batch.append(torch.from_numpy(index[batch_id]).int())
    ...
    return coords_batch, feats_batch, labels_batch, transformations_batch, pc_batch, pc_labels_batch, data_name_batch, index_batch
