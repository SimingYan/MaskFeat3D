...
from lib.metrics import ConfusionMatrix
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    nparameters = sum(p.numel() for p in base_model.parameters())
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    cm = ConfusionMatrix(num_classes=config.model.cls_dim)
    ...
    with torch.no_grad():
        ...
        for idx in range(iter_per_epoch):
            ...
            cm.update(logits.argmax(dim=1), label)
        ...
        print_log('[Validation] EPOCH: %d OA = %.4f  acc = %.4f' % (epoch, overallacc, macc), logger=logger)
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        ...
        tp, count = cm.tp, cm.count
        macc, overallacc, accs = cm.cal_acc(tp, count)
        print_log('[TEST] OA = %.4f  acc = %.4f' % (overallacc, macc), logger=logger)
...
def test_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
    ...
    with torch.no_grad():
        ...
        tp, count = cm.tp, cm.count
        macc, overallacc, accs = cm.cal_acc(tp, count)
        print_log('[TEST_VOTE] OA = %.4f  acc = %.4f' % (overallacc, macc), logger=logger)
...
def validate_detail(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    # print_log(f"[VALIDATION] Start validating epoch {epoch}", logger = logger)
    base_model.eval()  # set model to eval mode
    ...
    modelid = open('meta/modelnet40_shape_names.txt').readlines()

    for i in range(len(modelid)):
        shape_id2name[i] = modelid[i].strip()
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            logits, feat = base_model(points)
            target = label.view(-1)

            pred = logits.argmax(-1).view(-1)

            test_pred.append(pred.detach())
            test_features.append(feat.detach())
            test_label.append(target.detach())
        ...
        for i in range(test_label.max() + 1):
            cat_idx = test_label == i
            cat_acc = (test_pred[cat_idx] == test_label[cat_idx]).sum() / float(test_label[cat_idx].size(0)) * 100
            print('Category %s:' % shape_id2name[i], 'correct:', (test_pred[cat_idx] == test_label[cat_idx]).sum().data.cpu().numpy(), 'total:', cat_idx.sum().data.cpu().numpy(), 'acc:', cat_acc.data.cpu().numpy())
        
        test_embedded = TSNE(n_components=2, perplexity=100).fit_transform(test_features.data.cpu().numpy())
        NUM_CLASSES = 40 
        plt.figure(figsize=(16, 16))
        plt.scatter(test_embedded[:, 0], test_embedded[:, 1], c=test_label.data.cpu().numpy(), cmap=plt.cm.get_cmap("jet", NUM_CLASSES))
        plt.colorbar(ticks=range(1, NUM_CLASSES + 1))
        plt.clim(0.5, NUM_CLASSES + 0.5)
        plt.savefig('pc_tsne.png')
