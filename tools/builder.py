...
from tools.data_sampler import InfSampler, DistributedInfSampler
...
def dataset_builder_update(args, config):
    dataset = build_dataset_from_cfg(config._base_, config.others)
    shuffle = config.others.subset == 'train'
    if args.distributed:
        #sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle = shuffle)
        sampler = DistributedInfSampler(dataset, shuffle = shuffle)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size = config.others.bs,
                                            num_workers = int(args.num_workers),
                                            drop_last = config.others.subset == 'train',
...
def build_opti_sche(base_model, config):
    ...
    if opti_config.type == 'AdamW':
        ...
        optimizer = optim.SGD(base_model.parameters(), **opti_config.kwargs)
    else:
...
def save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, logger = None, Global_D=None, Local_D=None):
    if args.local_rank == 0:
        save_dict = {
                    'base_model' : base_model.state_dict() if args.distributed else base_model.state_dict(),
        ...
        if Global_D:
            save_dict['Global_D'] = Global_D.state_dict()
        if Local_D:
            save_dict['Local_D'] = Local_D.state_dict()

        torch.save(save_dict, os.path.join(args.experiment_path, prefix + '.pth'))
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    if state_dict.get('model') is not None:
        ...
        base_ckpt = {k.replace("module.", ""): v for k, v in state_dict['base_model'].items()}
    elif state_dict.get('model_state_dict') is not None:
        base_ckpt = {k: v for k, v in state_dict['model_state_dict'].items()}
    else:
